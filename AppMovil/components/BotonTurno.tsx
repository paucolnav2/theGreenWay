...
export default function BotonTurno({ idUsuario }: any) {
  ...
  const { activo, mensaje, alternar } = useRastreador(idUsuario);
    ...
    <View className="w-full flex-1 items-center justify-center relative">
        ...
        {/* Boton ubi*/}
     <TouchableOpacity 
        className={`w-64 h-64 justify-center items-center rounded-full border-8 shadow-xl ${
          activo ? 'bg-red-500 border-red-100' : 'bg-[#32CD32] border-gray-100'
        }`}
        onPress={alternar}
        activeOpacity={0.7}
      >
